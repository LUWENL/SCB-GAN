...
def setup_training_loop_kwargs(
        ...
        mineaug=None, # use Mineaug to make training robust default = False
        augmentations_list=None, # Data augmentations selected by MineAug
    ...
    # -----------------------------------
    # Dataset: data, cond, subset, mirror, mine_aug, aug_list
    # -----------------------------------
    ...
    args.training_set_kwargs = dnnlib.EasyDict(class_name='training.dataset.ImageFolderDataset', path=data,
                                               use_labels=True, max_size=None, xflip=False, mine_aug=mineaug,
                                               aug_list=augmentations_list)
    ...
    cfg_specs = {
        ...
        'stylegan2': dict(ref_gpus=4, kimg=25000, mb=16, mbstd=4, fmaps=1, lrate=0.002, gamma=10, ema=10, ramp=None,
                          map=8),  # Uses mixed-precision, unlike the original StyleGAN2.
        'paper256': dict(ref_gpus=2, kimg=25000, mb=16, mbstd=4, fmaps=0.5, lrate=0.002, gamma=1, ema=10, ramp=None,
                         map=8),
        'paper512': dict(ref_gpus=4, kimg=25000, mb=16, mbstd=4, fmaps=1, lrate=0.0025, gamma=0.5, ema=20, ramp=None,
                         map=8),
        'paper1024': dict(ref_gpus=4, kimg=25000, mb=16, mbstd=4, fmaps=1, lrate=0.002, gamma=2, ema=10, ramp=None,
                          map=8),
        'cifar': dict(ref_gpus=2, kimg=100000, mb=64, mbstd=32, fmaps=1, lrate=0.0025, gamma=0.01, ema=500, ramp=0.05,
                      map=2),
        'paper256_2fmap': dict(ref_gpus=2, kimg=25000, mb=16, mbstd=4, fmaps=1, lrate=0.002, gamma=1, ema=10, ramp=None,
                               map=8),
    }
    ...
    if cv is not None:
        ...
        args.cvD_kwargs = dnnlib.EasyDict(class_name='vision_module.cv_discriminator.Discriminator', cv_type=cv,
                                          output_type=output)
        ...
        if augcv is not None:
            if 'noaug' in augcv:
                ...
                if augpipecv is None:
                    augpipecv = 'bgc'
                args.augment_kwargs_cv = dnnlib.EasyDict(class_name='training.augment.AugmentPipe',
                                                         **augpipe_specs[augpipecv])
...
def get_args_parser():
    ...
    parser.add_argument('--metrics', help='Comma-separated list or "none" [default: fid50k_full]',
    ...
    # Base config.
    parser.add_argument('--cfg', help='Base config [default: auto]', type=click.Choice(
        ['auto', 'stylegan2', 'paper256', 'paper256_2fmap', 'paper512', 'paper1024', 'cifar']))
    ...
    # Discriminator augmentation.
    parser.add_argument('--diffaugment',
                        help='Comma-separated list of DiffAugment policy [default: color,translation,cutout]', type=str)
    ...
    parser.add_argument('--warmup',
                        help='if training from scratch, train with standard adversarial loss for 500k images and then introduce vision-aided-loss',
    ...
    parser.add_argument('--exact-resume',
                        help='0: only resume model weights, 1: resume model weights ensuring exact match and augpipe, 2: resume model weights, optimizer and augpipe',
                        type=int, default=0, metavar='INT')

    # miscellaneous
    parser.add_argument('--wandb-log', help='wandb logging instead of tensorboard logging', type=bool, metavar='BOOL',
                        default=False)
    parser.add_argument('--clean', help='FID evaluation using clean-fid (https://github.com/GaParmar/clean-fid)',
                        type=bool, metavar='BOOL', default=False)
...
def main(args):
    ...
    print(f'Dataset mineaug:    {args.training_set_kwargs.mine_aug}')
    print(f'Dataset augmentations list:    {args.training_set_kwargs.aug_list}')
